package wallmanager.ui.commons;
...
/**
 * Based on the component of SwingX : {@link https://java.net/projects/swingx/sources/svn/content/trunk/swingx-core/src/main/java/org/jdesktop/swingx/border/DropShadowBorder.java?rev=4316}
 */
public class DropShadowBorder implements Border, Serializable {
	...
	public void paintBorder(Component c, Graphics graphics, int x, int y, int width, int height) {
		...
		try {
			...
			// being
			// drawn. For instance, if the left & bottom shadows are being
			// drawn, then
			// the left shadow extends all the way down to the corner, a corner
			// is drawn,
			// and then the bottom shadow begins at the corner. If, however,
			// only the
			// bottom shadow is drawn, then the bottom-left corner is drawn to
			// the
			// right of the corner, and the bottom shadow is somewhat shorter
			// than before.

			int shadowOffset = 2; // the distance between the shadow and the
									// edge
