...
import wallmanager.ui.graphics.ImageScalerUtils;
...
public class ScreenView extends JComponent {
	...
	private WallpaperParameters parameters;
	...
	public ScreenView(final Screen screen) {
		this.screen = screen;
		this.parameters = new WallpaperParameters();
	...
	public void setParameters(WallpaperParameters parameters) {
		this.parameters = (parameters != null) ? parameters : new WallpaperParameters();
		rebuildImage();
	}
	...
	private class DisplayedImageBuilder extends SwingWorker<Void, BufferedImage> {
		...
		private void paintWallpaper(Graphics2D g2) throws IOException {
			// Read original file
			final BufferedImage originalImage = ImageIO.read(new File(parameters.getImage()));

			// Scale the image
			final BufferedImage scaledImage = ImageScalerUtils.getScaledImage(originalImage, parameters.getScalingAlgorithm(), containerDimension);
